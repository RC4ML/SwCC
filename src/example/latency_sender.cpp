...
int main() {
    ...
    cpu_start = 0;
    std::cout << "After the receiver is launched, enter any value to begin the latency test." << std::endl;
    ...
    size_t idx_25 = std::min(static_cast<size_t>(0.25 * n), n - 1);
    size_t idx_50 = std::min(static_cast<size_t>(0.50 * n), n - 1);
    size_t idx_75 = std::min(static_cast<size_t>(0.75 * n), n - 1);
    size_t idx_95 = std::min(static_cast<size_t>(0.95 * n), n - 1);

    fmt::println("5th percentile: {} ns", delay[idx_5]);
    fmt::println("25th percentile: {} ns", delay[idx_25]);
    fmt::println("50th percentile (median): {} ns", delay[idx_50]);
    fmt::println("75th percentile: {} ns", delay[idx_75]);
    fmt::println("95th percentile: {} ns", delay[idx_95]);
